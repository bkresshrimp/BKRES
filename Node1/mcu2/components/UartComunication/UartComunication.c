...
#include "esp_log.h"
#include "string.h"
const char *TAG = "Uart_Comuniocation";
char received_message[RD_BUF_SIZE];
...
void uart_receive_data(char* data, size_t max_length)
{
    ...
    if (length > 0) 
    {
        int read_bytes = uart_read_bytes(UART_NUM, (uint8_t*)data, 56, pdMS_TO_TICKS(20));
        if (read_bytes > 0) {
            data[read_bytes] = '\0'; // Null-terminate the received data
            strcpy(received_message, (char*)data);
