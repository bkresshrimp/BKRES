...
#define BIT_SIM_CREATE (1 << 0)

TaskHandle_t xTaskHandleSIM = NULL;
...
void TaskSIM(void *arg)
{
	...
	{
	ESP_LOGI(pcTaskGetName(NULL), " Task SIM waiting for BIT_SIM_CREATE");
	...
	if((uxBits & BIT_SIM_CREATE) == BIT_SIM_CREATE)
	{
		ESP_LOGI(pcTaskGetName(NULL), " Task SIM received BIT_SIM_CREATE");
...
void task_primary(void *pvParameters)
{
	...
	while(1) 
	{
		...
		if(send_len > 0)
		{
		ESP_LOGI(maintask, "Lora Node 1: %s", message);
		...
		while(waiting1) 
		{
			...
			if(count_timeout > 21)
			{
				if (eTaskGetState(xTaskHandleSIM) == eSuspended)
                {
                    vTaskResume(xTaskHandleSIM);
                    ESP_LOGI(maintask, "Resume task xTaskHandleSIM.");
                }
				xEventGroupSetBits(xEventGroup, BIT_SIM_CREATE);
				lora_receive(); // put into receive mode
				if (lora_received())
				{
				if (eTaskGetState(xTaskHandleSIM) != eSuspended)
                {
                    vTaskSuspend(xTaskHandleSIM);
                    ESP_LOGI(maintask, "Suspended task xTaskHandleSIM.");
                }
				count_timeout = 0;
				}				
				ESP_LOGI(pcTaskGetName(NULL), "COUNT TIMEOUT=%d", count_timeout);
				ESP_LOGW(pcTaskGetName(NULL), " Timeout,Start using SIM and ESP-NOW");
				ESP_LOGI(pcTaskGetName(NULL), "Task Lora setting BIT_SIM_CREATE");
...
void app_main()
{
    ...
    xTaskCreatePinnedToCore(&TaskSIM, "TaskSIM", 4096, NULL, 5, &xTaskHandleSIM, tskNO_AFFINITY); 
    example_espnow_init();
