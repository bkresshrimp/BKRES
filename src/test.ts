...
import { testFptMailConnection, getEmails } from './services';
import { logger } from './logger';
import { FptMailConfig } from './interfaces';

async function testFptMail(): Promise<void> {
  ...
  for (const config of fptMailConfig) {
    ...
    try {
      ...
      const connectionResult = await testFptMailConnection(config, dateRunning);
      ...
      if (connectionResult.success) {
        ...
        const emails = await getEmails(config, dateRunning);
      ...
      logger.error(`Test error for [${config.tenant}]:`, error);
    }
