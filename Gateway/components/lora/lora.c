 ...
 */
bool isSignificantChangeaboutNowTick(float TICK_current, float TICK_prev, float threshold) 
{
    return (fabs(TICK_current - TICK_prev) > threshold);
...
lora_init(void)
{
   ...
   gpio_reset_pin(CONFIG_CS_LORA);
   gpio_set_direction(CONFIG_CS_GPIO, GPIO_MODE_OUTPUT);
   gpio_set_level(CONFIG_CS_LORA, 1);
